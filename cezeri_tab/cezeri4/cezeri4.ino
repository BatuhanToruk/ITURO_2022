...
byte s1 = 39, s2 = 41, s3 = 43, s4 = 45, s5 = 47, s6 = 49, s7 = 51, s8 = 53,s9=23; //Sensörler
byte buton = 52;// Açma Kapaama Düğmesi Durumu
byte buzz = 4; //buzzer
...
int led=50;
//---------Hedef--Konum----------------------------------------------
byte birak[] = {33,  32,  14,  32,  34, 42, 24, 44};
...
//----------Çizgi--Takip------------------
const int kp = 20;//35    // 25   PID Motor Controller 
const int kd = 55;    // 50
const int maxt = 300;
const int gecis = 250;
int anahiz=250;
...
void setup() {
	...
	servo.attach(2);
	servo1.attach(9);	//----Motor Pinleri Çıkışı---------
	pinMode(INA, OUTPUT);
	pinMode(INB, OUTPUT);
...
// servo.write(130); servo1.write(50);
	//sirala();
	//siraYazdir();
//dataBekle();
...
void sagaDon(){
	...
	sensorRead();while (s_s8 != i) {s_s8 = digitalRead(s8);setMotor(240, -140);}
  sensorRead();while(syc>2) {sensorRead();setMotor(240, -140);}delay(20);
	sensorRead();while(s_s4 != i) {s_s4 = digitalRead(s4);setMotor(180, -90);} 
	//delay(20);                 // 4.Sensör Siyah Çizğiyi Görmediği Sürece YAVAŞÇA Dön
	setMotor(-90, 90); delay(30);	setMotor(0, 0); delay(200);
	yon ++;  	if (yon == 4)  yon = 0;
	}
void solaDon(){
	
	setMotor(0, 0); delay(500);
	sensorRead();while (s_s1 != i) {s_s1 = digitalRead(s1);setMotor(-240, 140);}
  sensorRead();while(syc>2) {sensorRead();setMotor(-240, 140);}	delay(20);
	sensorRead();while (s_s5 != i) {s_s5 = digitalRead(s5);setMotor(-180, 90);} 
	//delay(20);
	setMotor(50, -50); delay(30);	setMotor(0, 0); delay(200);
	yon --;	if (yon < 0) yon = 3;
	}
...
void cizgiTakip(){
	sensorRead();
	sol1 = (240 + total); sag1 = (140 - total);
//  if(total<0){
//    sol1 = (anahiz + total); sag1 = (anahiz);
//    
//    }
//   else{
//    sol1 = (anahiz ); sag1 = (anahiz - total);
//    }
...
void kareGec(){
    i=1;  sensorRead();while(syc<4) {cizgiTakip();}   setMotor(240,140); delay(150);//azGit(); // satir gir
    
   i=0;  sensorRead();  while(syc<4) {cizgiTakip();}  setMotor(240,140); delay(150);//setMotor(0,0);delay(2000);//azGit(); // satir ortayagel
     
   i=0;  sensorRead();  while(syc<4) {cizgiTakip();}  setMotor(240,140); delay(150);// azGit(); // satir cik
    neredeyim();
...
void yeniBirak(){
  servo.write(10); servo1.write(170);delay(600);
    setMotor(-240,-140);delay(500);
    Serial1.println("kutu birakildi");
    setMotor(0,-30);delay(30);setMotor(0,0);delay(1000);
    solaDon();
    yon=2;
   // satir--;
   // neredeyim();
...
//		else if (yon == 2) satir ++;
	...
	if(sutun>H_sutun){                              
      ...
      if(kutu_durum==0){
        ...
        kutuAlGel();
        setMotor(0,-30);delay(30);setMotor(0,0);delay(500);
...
void azGit() {
	setMotor(0,-70); delay(40);
	setMotor(240,140); delay(330);
	setMotor(0,0); delay(500);}
